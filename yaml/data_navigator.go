...
func writeMap(context interface{}, paths []string, value interface{}) yaml.MapSlice {
	pscfLog.Log.Debugf("writeMap for %v for %v with value %v\n", paths, context, value)
	...
	if child == nil {
		...
		child = entryInSlice(mapSlice, paths[0])
		pscfLog.Log.Debugf("\tAppended child at %v for mapSlice %v\n", paths[0], mapSlice)
	}

	pscfLog.Log.Debugf("\tchild.Value %v\n", child.Value)

	remainingPaths := paths[1:]
	child.Value = updatedChildValue(child.Value, remainingPaths, value)
	pscfLog.Log.Debugf("\tReturning mapSlice %v\n", mapSlice)
	return mapSlice
}
...
func writeArray(context interface{}, paths []string, value interface{}) []interface{} {
	pscfLog.Log.Debugf("writeArray for %v for %v with value %v\n", paths, context, value)
	...
	pscfLog.Log.Debugf("\tcurrentChild %v\n", currentChild)

	remainingPaths := paths[1:]
	array[index] = updatedChildValue(currentChild, remainingPaths, value)
	pscfLog.Log.Debugf("\tReturning array %v\n", array)
	return array
}
...
func deleteMap(context interface{}, paths []string) yaml.MapSlice {
	pscfLog.Log.Debugf("deleteMap for %v for %v\n", paths, context)
	...
	if len(remainingPaths) > 0 {
		...
		pscfLog.Log.Debugf("\tDeleted item index %d from mapSlice", index)
	}

	pscfLog.Log.Debugf("\t\tlen: %d\tcap: %d\tslice: %v", len(mapSlice), cap(mapSlice), mapSlice)
	pscfLog.Log.Debugf("\tReturning mapSlice %v\n", mapSlice)
	return newSlice
}

func deleteArray(context interface{}, paths []string, index int64) interface{} {
	pscfLog.Log.Debugf("deleteArray for %v for %v\n", paths, context)
	...
	if len(remainingPaths) > 0 {
		...
		pscfLog.Log.Debugf("\tDeleted item index %d from array, leaving %v", index, array)
	}
...
func deleteChildValue(child interface{}, remainingPaths []string) interface{} {
	pscfLog.Log.Debugf("deleteChildValue for %v for %v\n", remainingPaths, child)

	idx, nextIndexErr := strconv.ParseInt(remainingPaths[0], 10, 64)
	if nextIndexErr != nil {
		// must be a map
		pscfLog.Log.Debugf("\tdetected a map, invoking deleteMap\n")
		return deleteMap(child, remainingPaths)
	}

	pscfLog.Log.Debugf("\tdetected an array, so traversing element with index %d\n", idx)
